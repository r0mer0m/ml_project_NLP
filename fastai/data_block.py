...
class ItemList():
    ...
    def get_label_cls(self, labels, label_cls:Callable=None, sep:str=None, labelwords=None, **kwargs):
        "Return `label_cls` or guess one from the first element of `labels`."
        if labelwords is not None:              return MultipleCategories
    ...
    def label_labeler(self, cols:IntsOrStrs=0, sep=',', **kwargs)->'LabelList':
        "Label `self.items` from the values in `cols` in `self.xtra`."
        labels = _maybe_squeeze(self.xtra.iloc[:,df_names_to_idx(cols, self.xtra)])
        labels = array(labels, dtype=object)
        # labels = [label.split(sep) for label in labels]
        label_cls = self.get_label_cls(labels, labelwords=True, **kwargs)
        y = label_cls(labels, sep=',', **kwargs)
        res = self._label_list(x=self, y=y)
        return res
...
def labeler_loss(out,y):
    tot_sum = 0
    for i in range(out.shape[0]):
        tot_sum = tot_sum + F.cross_entropy(out[i, :, :], y[i, :])
    return tot_sum



class MultipleCategories(ItemList):
    "When labels is vector of integers"
    def __init__(self, items:Iterator, classes:Collection=None, sep=',', **kwargs):
        if sep is not None: items = array([[int(element) for element in item.split(',')] for item in items])
        self.classes = list(set([it for item in items for it in item]))
        self.c = len(self.classes)
        self.loss_func = labeler_loss
